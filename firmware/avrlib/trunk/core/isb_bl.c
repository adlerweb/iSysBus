 ...
 *
 * @date		$Date: 2012-06-08 15:32:40 +0200 (Fr, 08. Jun 2012) $
 *
 * @version		$Rev: 8199 $
 *
 * @note
 ...
 */
	...
	#define ISB_BL_C_
		...
		const uint8_t Isb_Bootloader_Code[4] = { 0x00, 0x00, 0x00, 0x00 };
		...
		#include "./../lib/isb_protocol_def.h"

	#endif /* USE_AVRLIB_ISB */
	...
	void get_ResetSource(void)
	{
		...
		{
			sendchar(ISB_PROTOCOL_STARTBYTE);
			sendchar(ISB_PROTOCOL_TOF);
			sendchar((Msg->Flags & ISB_MSG_FLAGS_DLC_MASK) + 5);
			sendchar(Msg->Flags);
			...
			sendchar(ISB_PROTOCOL_STOPBYTE);
		}
	#endif /* ISB_INF == USART0 */
		...
		{
			IF_CAN0_SS_PORT &= ~(1<<IF_CAN0_SS_PORTX);
	...
	int main(void)
	{
		...
		if (Isb_Node_Operating_Mode == ISB_NODE_OPERATING_MODE__BL_FLASH)
		{
		...
		#if ISB_INF == CAN0
			IF_CAN0_SS_DDR |= (1<<IF_CAN0_SS_DDX);
			IF_CAN0_SS_PORT |= (1<<IF_CAN0_SS_PORTX);
			Inf_Spi_Init();
			IF_CAN0_SS_PORT &= ~(1<<IF_CAN0_SS_PORTX);
			...
			#else
				IF_CAN0_RS_DDR |= (1<<IF_CAN0_RS_DDX);
				IF_CAN0_RS_PORT &= ~(1<<IF_CAN0_RS_PORTX);
			#endif /* ISB_INF_CAN0_RS */
		...
		for (;;)
		{
			...
			wdt_reset();
				...
				#error "No definition for this MCU available."
					...
					#error "No definition for this MCU available."
				#endif
				// BusProtokoll - Startbyte
				if (relPos == 0 && tempByte == ISB_PROTOCOL_STARTBYTE)
				{
					relPos++;
					continue;
				}
				// BusProtokoll - Type Of Frame
				if (relPos == 1 && tempByte == ISB_PROTOCOL_TOF)
				{
					relPos++;
					continue;
				}
				// BusProtokoll - Datenlänge
				if (relPos == 2)
				{
					relPos++;
					continue;
				}
				// BusProtokoll - Daten -> iSysBus - Flags
				if (relPos == 3)
				{
				...
				// BusProtokoll - Daten -> iSysBus - DestId
				if (relPos == 4)
				{
				...
				// BusProtokoll - Daten -> iSysBus - SourceId
				if (relPos == 6)
				{
				...
				// BusProtokoll - Daten -> iSysBus - Databytes 0-8
				if ((relPos > 8) && (relPos <= (Isb_Msg_Dlc + 6)))
				{
					Isb_Msg.Data[relPos - 7] = tempByte;
					relPos++;
					continue;
				}
				// BusProtokoll - Prüfsumme
				...
				// BusProtokoll - Stopbyte
				if (relPos == (Isb_Msg_Dlc + 9))
				{
					relPos = 0;
					if (tempByte != ISB_PROTOCOL_STOPBYTE)
					{
						continue;
					}
				}
			...
			{
				if (Isb_Msg.DestId == ISB_MSG_MULTICAST_GROUPID_SETNODEID)
				{
					if (Isb_Msg.Data[0] == ISB_MSG_TOP__SERIAL_NUMBER__NODEID)
					{
